# Chrono Trigger Resume Checklist — `C6:FE00..C7:07FF`

## Current resume state
- working branch: `live-work-from-pass166`
- ROM: `rom/Chrono Trigger (USA).sfc`
- practical live seam: `C6:FE00..`
- last continuation note in force: `docs/sessions/chrono_trigger_session15_continue_notes_58.md`
...
- the long run from `C5:3B00` through `C6:FDFF` has still produced **0 promotions**
- do **not** go into the `C6 -> C7` bank turn assuming the new bank automatically becomes clean code territory
...
Acceptable outcomes:
- all ten pages freeze honestly and the seam advances to `C7:0800..`
...
1. Read `reports/c6_fe00_c7_07ff_seam_block.md` first.
...
4. For each manual-review page, run:
   ```bash
   python3 tools/scripts/score_target_owner_backtrack_v1.py --rom 'rom/Chrono Trigger (USA).sfc' --range 'CB:XXXX..CB:XXFF' --json > reports/cb_xx00_xxff_backtrack.json
   ```
5. Run anchor reports only for targets that still look structurally defensible after byte review:
   ```bash
   python3 tools/scripts/build_call_anchor_report_v3.py --rom 'rom/Chrono Trigger (USA).sfc' --target CB:XXXX --manifests-dir passes/manifests --closed-ranges-snapshot tools/cache/closed_ranges_snapshot_v1.json --only-valid
   ```
...
- the block yields no promotions and the new live seam is cleanly `C7:0800..`
