# Chrono Trigger Resume Checklist — `C6:9A00..C6:A3FF`

## Current resume state
- working branch: `live-work-from-pass166`
- ROM: `rom/Chrono Trigger (USA).sfc`
- practical live seam: `C6:9A00..`
- last continuation note in force: `docs/sessions/chrono_trigger_session15_continue_notes_48.md`
...
- effective closed-range snapshot after refresh: `tools/cache/closed_ranges_snapshot_v1.json` (`65` manifest-backed ranges + `590` frozen continuation-note pages)

Important reality:
- the continuation frontier is note-backed, not manifest-backed
- frozen continuation-note pages now feed caller-context scoring through the seam snapshot layer
- the long run from `C5:3B00` through `C6:99FF` produced **0 promotions**
- do **not** go into `C6:9A00..A3FF` assuming the lane is about to turn clean
...
Acceptable outcomes:
- all ten pages freeze honestly and the seam advances to `C6:A400..`
...
1. Read `reports/c6_9a00_a3ff_seam_block.md` first.
...
- the block yields no promotions and the new live seam is cleanly `C6:A400..`
...
- `docs/sessions/chrono_trigger_session15_continue_notes_49.md`
