...
def main() -> int:
    parser = argparse.ArgumentParser(description='Score incoming raw xrefs into a SNES range using caller/target context, bad-start gating, dead-lane suppression, and coarse page-family tagging')
    ...
    for item in iter_raw_callers(rom_bytes):
        ...
        eff = effective_strength(anchor, caller_risk, target_risk, start_class, dead_match is not None)
    ...
    if args.json:
        ...
        for hit in hits:
            ...
            start_text = f" start={hit['start_byte']} {hit['start_reason']}" if hit['start_reason'] else f" start={hit['start_byte']}"
            print(f"  {hit['caller']} {hit['kind']} -> {hit['target']} | anchor={hit['anchor_strength']} effective={hit['effective_strength']} caller_risk={hit['caller_risk']} target_risk={hit['target_risk']}{start_text}{dead_text}")
            if hit['caller_range']:
